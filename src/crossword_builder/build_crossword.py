...
BoardVariable = namedtuple('BoardVariable', ['i', 'j', 'index', 'orientation', 'length'])
PromptVariable = namedtuple('PromptVariable', ['orientation', 'index', 'prompt', 'length'])
...
class Variable():
    ...
    def __init__(self, pos: Position, index: int, orientation: str, length: Union[int, Tuple[int, int]], prompt: str) -> None:
    ...
    def validate(self) -> bool:
        ...
        if isinstance(self.length, int) and self.length < 3:
            print("length < 3")
            return False
        if isinstance(self.length, tuple) and (self.length[0] + self.length[1] < 3):
            print("length < 3")
            return False
        # check the prompt
        ...
        if not isinstance(self.pos.i, int) or not isinstance(self.pos.i, int) or not isinstance(self.index, int):
    ...
    def setup_cells(self) -> None:
        ...
        if self.orientation == ACROSS:
            self.cells = [(self.i, self.pos.j + k) for k in range(self.length)]
        elif self.orientation == DOWN:
            self.cells = [(self.pos.i + k, self.pos.j) for k in range(self.length)]
        return
    ...
    def __hash__(self) -> int:
        hash((self.pos, self.orientation, self.length))
        ...
        for j in range(cols):
            ...
            if is_start_of_variable(board, Position(i, j)):
                for orientation in is_start_of_variable(board, Position(i, j)):
                    length = get_variable_length(board, Position(i, j), orientation)
                    if length > 1:
                        board_variables.append(BoardVariable(i, j, index, orientation, length))
                        appended = True
                if appended:
                    index += 1
...
def read_prompts(filepath: str) -> List[PromptVariable]:
    ...
    for _, row in df.iterrows():
        ...
        prompts.append(PromptVariable(orientation, index, row['prompt'], length))
    return prompts


def match_prompt_to_position(board_var: BoardVariable, prompt: PromptVariable) -> bool:
...
def combine_board_and_prompts(board_vars: List[BoardVariable], prompts: List[PromptVariable]) -> List[Variable]:
    variables = []
    for board_var in board_vars:
        for prompt in prompts:
            if match_prompt_to_position(board_var, prompt):
                variables.append(Variable(Position(board_var.i, board_var.j), board_var.index,
                                 board_var.orientation, board_var.length, prompt.prompt))
